...
> 说明：每门课的 **课程简介 · 主要知识点 · 成绩组成** 依据 UNSW **2026 官方 handbook / course outline** 整理；**整体难度 · 建议学习路线** 为 **主观经验参考**（因人而异）。UNSW 各 term 的评估占比常有微调，成绩组成最终一律以当学期 course outline 为准。来源见页面底部。
...
- **整体难度**：⭐⭐⭐☆☆ — 概念本身不难，但**指针与动态内存**是初学者的第一道坎；节奏快，每周 lab + 作业积累起来负担不小。
- **成绩组成**：Problem Sets（每周 lab / 小测）15% + Assignment 1 20% + Assignment 2 25% + 期末考试 40%（以当学期 course outline 为准）。
- **建议学习路线**：作为学位第一门编程课，零基础也能选，不用担心没学过 C。真正想让自己学得轻松，开学前花点时间摸熟命令行和 Linux 基本操作会很加分；上课后每周 lab 当周就清掉、别攒到考前，遇到指针把它画成"内存图"就没那么可怕了。学完你会很自然地接上 COMP1521 和 COMP2521。
...
- **整体难度**：⭐⭐⭐☆☆ — 技术难度中等，真正的坎是**小组协作与分工**：项目分 3 个迭代（iteration 2 工作量最大），队友配合和进度管理往往比写代码更棘手。
- **成绩组成**：以贯穿全程的分迭代小组项目（含 Python 后端实现、测试、Git 使用与团队协作评分）为主，辅以个人 lab / 练习与实验室评估（务必以当学期 course outline 公布的具体占比为准）。
- **建议学习路线**：修过 COMP1511 就可以放心选，这门课技术上不会难住你，最考验的其实是小组协作。想学得顺，开学第一周就把 Git 和团队仓库配好、养成写测试的习惯；主动认领迭代任务、别当"隐形队友"，你的体验会好很多。它也为之后的 COMP2511、COMP3900 / COMP9900 打底。
...
- **整体难度**：⭐⭐⭐☆☆ — 单点知识不难，但**Shell 与正则的"奇技淫巧"**容易踩坑；两个较大的编程作业需要把零散工具组合成完整方案。
...
- **建议学习路线**：如果你已经修过 COMP1511 / COMP1521、对命令行不陌生，选它会上手很快。想学得省力，平时就多在终端敲 grep / sed / awk 和正则，作业才不会被"工具怎么拼在一起"卡住。这是一门性价比很高的课，练熟了能实打实提升你日后的开发效率。
...
- **整体难度**：⭐⭐⭐⭐☆ — 从"能写代码"到"写好设计"是质变；**设计模式的意图与适用场景**是理解难点，结对大项目对设计能力和协作都是考验。
- **成绩组成**：以结对完成的大型 Java 项目（分阶段，考核设计模式与设计原则的应用）为主，辅以 lab / 练习与期末考试（考察对各设计模式意图、方案与实现的掌握），具体占比以当学期 course outline 为准。
- **建议学习路线**：修过 COMP1531 和 COMP2521 再来选最舒服。别被"设计模式"吓到，学的时候别死记结构，每个模式先想清楚它到底解决什么问题，你就会豁然开朗；做项目时先动脑设计再动手写代码，会省很多返工。它是软件工程方向的关键一课，直接撑起你高年级的项目课。
...
- **整体难度**：⭐⭐⭐⭐☆ — 函数式思维与命令式差异大，**"用类型和性质思考"**是最大门槛；期末成绩由平时实践分与期末考的调和平均决定，任一端偏弱都会明显拉低总分。
- **成绩组成**：每周小测（约 8 次，考察理论）+ 每周编程练习（约 7 次，含 1 次同行评审）+ 期末考试；**最终成绩为"实践部分"与"期末考试"的调和平均**（harmonic mean），具体占比以当学期 course outline 为准。
- **建议学习路线**：修过 COMP2521 再选会更从容。函数式思维和你习惯的写法差别不小，但别怕——开课前先摸一摸 Haskell 语法、重点体会"类型即规约"的思路，跟上节奏就能拿下。特别提醒：成绩是平时实践和期末的调和平均，短板会被放大，所以每周练习千万别欠账。你要是对编程语言理论、形式化方法感兴趣，这门课会很对味。
...
- **整体难度**：⭐⭐⭐⭐☆ — 对数学背景较弱的转专业研究生是明显的坎，**证明题（proof）**最容易失分；概念多、需要持续练习而非临时抱佛脚。
- **成绩组成**：期末考试 60% + 作业 30%（2 次，各约 10 分）+ 小测 / Quizzes 10%（约 10 次；另有每周 Formatif 练习）（以当学期 course outline 为准）。
- **建议学习路线**：没有严格的编程先修，但对数学的"手感"有要求，如果你是转专业、数学有点生，别慌，靠持续练习完全能补上来。每周把 Formatif 和练习题跟着做完，尤其多练证明题的书写规范，别临时抱佛脚；配合 Rosen《离散数学及其应用》或 Lehman《Mathematics for CS》一起看会更稳。它是你之后算法、理论类课程的地基。
...
- **整体难度**：⭐⭐⭐⭐☆ — **BWT 相关的编程作业**公认硬核，对内存 / 时间效率要求高，算法与工程实现能力缺一不可。
- **成绩组成**：两次编程作业（围绕 Web 数据压缩与搜索算法的实现）+ 期末考试（主要评估项）；具体占比以当学期 course outline 为准（注意：该课禁止提交由 Copilot / ChatGPT 等自动工具生成的代码）。
- **建议学习路线**：选它之前先掂量下自己的数据结构、算法和 C / C++ 底子（最好先修过 COMP9024 / COMP2521 类课程），这些够扎实你会学得轻松很多。最容易卡人的是 BWT 编程作业，对内存和时间效率要求高，一定要尽早动手、预留充足的调优时间，别拖到最后。注意这门课 2026 只在 T2 开。
...
- **整体难度**：⭐⭐⭐⭐☆ — 难点在于用 C 手动管理指针与内存去实现复杂结构（尤其图和树），概念不难但工程量与调试量大。
- **成绩组成**：Labs 15% + Quizzes 10% + Assignment 1 15% + Assignment 2 15% + 期末考试 45%（含期末及格线 hurdle；以当学期 course outline 为准）。
- **建议学习路线**：这门课几乎是决定你后续算法课上限的"内功课"，很值得认真对待。学起来省力的关键，是先把 C 的指针和动态内存彻底吃透，它是全课地基；每周 lab 和 quiz 当周清掉别欠账，因为知识点强累积，攒着最难受。两个大作业（通常和图、BST 有关）尽早动手、留足调试时间，从第一天就习惯用 gdb / valgrind 排查内存问题，你会少踩很多坑。
...
- **整体难度**：⭐⭐⭐⭐⭐ — 全校公认硬核，形式化证明 + Haskell 函数式思维双重门槛，数学基础薄弱会很吃力。
- **成绩组成**：Assignment 0（理论 / 证明）15% + Assignment 1（编程）17.5% + Assignment 2（编程）17.5% + 期末考试 50%（以当学期 course outline 为准）。
- **建议学习路线**：这是全校公认最硬核的课之一，选之前请对自己的数学和抗压能力有点数——但只要方法对，也没到完全啃不动的地步。想少受苦，务必提前预习 Haskell 和基础离散数学 / 逻辑证明，别等开课再临时学；把每周 lecture 的语义推导规则手抄成"规则卡片"反复练，会帮你稳住。证明型作业（Ass0）其实是期末的预演，一定独立做完并订正。课只在 T3 开，UG 走 COMP3161、PG 走 COMP9164，同堂上。
...
- **整体难度**：⭐⭐⭐☆☆ — 对有编程经验者偏友好，难点在于每周高频 quiz 的节奏与 Python 进阶语法的熟练度，而非概念深度。
- **成绩组成**：8 次每周编程 Quiz 共 24%（每次 3 分）+ 2 个编程 Assignment 共 26%（每个 13 分）+ 期末考试 50%（在线，最终成绩为各项算术平均，需达 50 分及格）（以当学期 course outline 为准）。
- **建议学习路线**：如果你有点编程经验，这门课其实挺友好，主要考验你对 Python 进阶语法的熟练度和每周 quiz 的跟进节奏。最高效的吸收方式是把课程给的 Jupyter notebook 逐格运行、先猜输出再验证；每周 quiz 当周做完保持手感，两个作业尽早开工（迟交每天扣分，别拖）；practice exercises 就当期末实战来练。
...
- **整体难度**：⭐⭐⭐⭐☆ — 与 COMP2521 相当，C 语言指针 / 内存实现是主要门槛，且期末考设有单独及格线。
...
- **建议学习路线**：这是很多 IT 硕士打通算法的必经一课，很值得好好学。C 语言的指针和内存实现是主要门槛，若你底子薄，开课前一定先补齐，之后会顺很多。每周 exercise 别拖，它们直接对应期末考点；Week 6 的 midterm 和 Week 10 的大作业提前规划好。期末占 60% 又有单独 hurdle，复习时务必把每类结构亲手默写实现一遍，别只看不写。
...
- **整体难度**：⭐⭐⭐☆☆ — 概念不深但覆盖面广，SQL 与 PLpgSQL 的实操作业容易在细节和效率要求上翻车。
...
- **建议学习路线**：概念不算深、覆盖面广，几乎是所有后续数据库课的地基，很值得选。想拿下它，先把 ER → 关系模式 → 规范化这条主线彻底吃透，这是设计题和考试的核心；SQL 别只看，多在本地 PostgreSQL 上敲，把 JOIN、聚合、子查询和 PLpgSQL 触发器写熟。作业容易在细节和效率上翻车，提早开工、留时间跑自动评测和调优，就不慌了。
...
- **整体难度**：⭐⭐⭐⭐☆ — 概念抽象、数学（信息论 / 概率 / 线性代数）与编程项目并重，programming project 工作量较大。
- **成绩组成**：Assignment（非编程）20% + Project（编程）30% + 期末考试 50%（期末为 hurdle，须 >40%；以当学期 course outline 为准）。
- **建议学习路线**：这是一门偏进阶的选修，概念抽象、数学和编程并重，选之前对信息论 / 概率 / 线代有点准备会更舒服。学的时候先把倒排索引和 TF-IDF / 向量空间排序这条主干啃透，后面的概率 / 语言模型都建立在它上面，主干稳了就不虚。编程 project 工作量不小，早启动、分模块实现并自测；评估指标（P / R / MAP / NDCG）一定练到会手算，考试常考。
...
- **整体难度**：⭐⭐⭐⭐⭐ — 需读懂并修改 C 语言写的 PostgreSQL 源码，工程量与底层理解要求都很高。
- **成绩组成**：Quizzes 15% + Assignment 1（新数据类型）15% + Assignment 2（数据库存储结构）20% + 期末考试 50%（期末为 hurdle，须 ≥40%；以当学期 course outline 为准）。
- **建议学习路线**：这是数据库方向最硬核的进阶课之一，你要直接改 PostgreSQL 的 C 源码，选之前请对工程量有心理准备——但别被吓退，方法对了完全可控。先把 C 和指针 / 内存管理补牢，再花时间读懂相关源码模块的调用结构。作业务必尽早开始、用提供的测试子集边写边验证；把存储 → 索引 → 查询处理 → 事务这条内部数据流串成一条线理解，你就不会一头雾水了。
...
- **整体难度**：⭐⭐⭐☆☆ — 概念门槛中等，但作业数量多、动手工程量大，节奏紧。
- **成绩组成**：多个作业 + 期末考（近年期末约占 50%，如某年为 Quizzes 10% + A1 10% + A2 10% + A3 20% + 期末 50%）；各学期权重变动较大，以当学期 course outline 为准。
- **建议学习路线**：偏应用、动手多，概念门槛中等，如果你喜欢做能跑起来的东西会很享受。想学得不累，先把 Python 和 Flask / REST 基础打牢，动手搭一个最小可运行 API 找感觉。作业密集且一环扣一环，务必按周跟进、别积压；A3 常涉及数据分析 / ML，提前熟悉 pandas 和基本建模流程，到时就轻松多了。
...
- **整体难度**：⭐⭐⭐☆☆ — 广度大于深度，概念多但每个点不算太深，难在编程作业（搜索 / 博弈 / 学习算法实现）和逻辑推理部分。
- **成绩组成**：Assignment 1 25% + Assignment 2 25% + 期末笔试 50%（COMP9814 比例通常一致但作业要求更高；以当学期 course outline 为准）。
- **建议学习路线**：这是 AI 的"入门总览课"，广度大于深度，很适合你先建立 AI 全局观再决定深入哪个分支。学起来省力的做法，是先把搜索（BFS / DFS / A*）和逻辑（命题 / 一阶逻辑、归结）这两块啃透，它们是笔试拿分核心。编程作业别拖，尤其博弈和强化学习实现题挺耗时，早动手才从容。
...
- **整体难度**：⭐⭐⭐☆☆ — 概念门槛中等，难点在工程调试、团队协作和让真机器人稳定运行；C++ / Python 编程能力是前提。
- **成绩组成**：期中项目 + 报告 40%（第 5 周演示）+ 期末项目演示 + 报告 60%（第 10 周演示，报告第 11 周末交）；小组演示、个人提交报告（以当学期 course outline 为准）。
- **建议学习路线**：几乎没有考试、全靠做出能跑的机器人评分，如果你喜欢动手,这门课会很好玩。前提是有点 C++ / Python 底子。想少踩坑，开学就立刻熟悉 ROS2 的基本概念（node / topic / tf）和 TurtleBot3 仿真环境。项目重在"能演示 + 讲清为什么这样跑"，尽早分工、留足调试时间，千万别等最后一周才开工。
...
- **整体难度**：⭐⭐⭐⭐☆ — 需要一定机器学习 / 深度学习基础，数学与编程要求较高，项目工作量大。
- **成绩组成**：据公开信息含每周 quiz、个人 assignment 与小组 project 三部分；官方 outline 需登录访问，具体占比未能核实，以当学期 course outline 为准。
- **建议学习路线**：想进 NLP / LLM 方向的话这门课很对口，但它对 ML / 深度学习基础、数学和编程都有要求，选之前先掂量下自己的底子。开课前把 Python + PyTorch 和基础 ML 补齐会让你轻松很多；每周 quiz 紧跟课堂、别积压；项目工作量大，尽早选题跑通 baseline，再慢慢迭代提升。
...
- **整体难度**：⭐⭐⭐☆☆ — 广度型课程，概念多；难点是搜索 / 推理的严谨性和作业编程，注意期末有 hurdle。
- **成绩组成**：Assignment 1 25%（第 5 周）+ Assignment 2 25%（第 9 周）+ 期末笔试 50%（及格 hurdle：期末须至少 20/50；以当学期 course outline 为准）。
- **建议学习路线**：这是不少 AI 方向研究生的第一门 AI 课，广度型、概念多，适合先建立全局观。复习时把搜索和逻辑 / 概率推理当笔试重点，务必冲过期末 hurdle，这点别大意。两个 Python 编程作业按时完成，注意代码风格和分析部分同样计分，别只顾着跑通结果。
...
- **整体难度**：⭐⭐⭐⭐☆ — 数学要求高（线代 / 概率 / 微积分 / 优化），理论推导与编程实现并重，是本类课程中较硬核的一门。
- **成绩组成**：Homework 1 15% + Homework 2 15% + 小组 Project 20% + 期末笔试 50%（及格线 50/100；以当学期 course outline 为准）。
- **建议学习路线**：这是 ML 方向绕不开的基石课，也是这类课里较硬核的一门，数学要求高，选之前先夯实线代与概率统计会让你舒服很多。学的时候跟着课程用 NumPy / sklearn 亲手实现算法，理解会深得多；两次 homework 是吃透内容的关键，别拖；project 当作小型科研来做，注意报告质量。
...
- **整体难度**：⭐⭐⭐⭐⭐ — 概率与数学要求非常高，推断算法抽象、期末含 hurdle，是本类课程最硬核的一门。
- **成绩组成**：Quizzes 10%（8 次取平均）+ Assignment 1 15%（第 5 周）+ Assignment 2 15%（第 9 周）+ 期末笔试 60%（hurdle：期末须 ≥24/60；以当学期 course outline 为准）。
- **建议学习路线**：这是本类课里最硬核的一门，概率与数学要求非常高，选之前请务必先修并巩固 COMP9417 和概率论，底子不牢会很吃力。学的时候把每周 quiz 当成逼自己跟进推断算法的抓手，别落下；把变量消元 / 联合树 / 采样这几套推断流程亲手推一遍。期末含 hurdle，不容有失，早准备。
...
- **整体难度**：⭐⭐⭐⭐☆ — 数学（梯度、优化）与 PyTorch 编程要求高，小组项目工作量大，理论与工程双重挑战。
- **成绩组成**：个人 Assignment 20% + 课堂参与 5% + 小组 Project 30%（第 10 周展示）+ 期末考试 45%（以当学期 course outline 为准）。
- **建议学习路线**：这是通往深度学习研究的核心课，数学（梯度、优化）和 PyTorch 编程要求都不低，选之前有点准备会更从容。上手前先熟练 PyTorch 的张量与自动求导，把反向传播亲手推一遍，很多东西就通了。小组项目第 4 周前组好队、尽早定题跑通 baseline；期末占比高，理论概念要系统复习，别只顾着调模型。
...
- **整体难度**：⭐⭐⭐⭐☆ — 单点难度不极端，但要求综合运用多种 AI 技术且全为团队项目，协作与工程整合能力是最大挑战；需先具备 ML / DL 基础。
- **成绩组成**：项目提案报告 10%（第 3 周）+ 文献综述展示 20%（第 5 周）+ 项目 demo 30%（第 10 周，含参与分）+ 项目报告 40%（第 11 周）；全部为小组评估（以当学期 course outline 为准）。
- **建议学习路线**：这是一门整合型高阶项目课，几乎全靠小组项目评分，适合你已经有 ML / DL 基础、想练综合实战时选，最好先修过 COMP9417 / 9444 打底。想不慌，第 2 周前就尽快组队选题，提案阶段就想清楚"要整合哪几类 AI 技术"。文献综述和最终报告合计占 60%，写作质量一定要用心，别让它拉后腿。
...
- **整体难度**：⭐⭐⭐⭐☆ — 需要一定数学与 Python / OpenCV 基础，项目工作量大，深度学习部分有难度。
- **成绩组成**：Lab 任务 10%（个人，前半学期每周一次共约 4 次）+ 小组 Project 40%（第 5–10 周）+ 期末考试 50%（以当学期 course outline 为准）。
- **建议学习路线**：这门课理论和动手兼顾，选之前有点数学和 Python / OpenCV 基础会更舒服。前半学期跟着每周 lab 把 OpenCV / 图像处理基础打牢，后面做项目就不慌；project 从第 5 周就开始，尽早组队定题、别拖。期末占一半，传统方法和深度学习原理都要系统复习，两头都别落。
...
- **整体难度**：⭐⭐⭐⭐☆ — 图算法的复杂度分析与大规模稀疏图上的工程实现是主要门槛，需要扎实的算法与数据结构基础。
- **成绩组成**：Assignment 1 15% + Assignment 2 10% + Project 25% + 期末考试 50%（期末与项目占比最重；以当学期 course outline 为准）。
- **建议学习路线**：这是数据科学方向里偏"结构化关系挖掘"的一门硬核课，需要扎实的算法与数据结构基础，选之前先把图论基础（BFS / DFS、最短路、连通分量）补齐会顺很多。两个 Assignment 侧重经典图算法实现，务必吃透复杂度分析；Project 通常是较大规模的图数据，早动手、注意内存和效率优化，别到后面被性能卡住。
...
- **整体难度**：⭐⭐⭐⭐☆ — 分布式"换脑子"的思维方式（把问题拆成 map / reduce）加上 Spark / Scala 环境配置，是初学者最大的坎；期末笔试还要求手写算法逻辑。
...
- **建议学习路线**：这是数据科学方向最经典的"大数据工程"课，最大的坎是把思维切换成分布式（把问题拆成 map / reduce）加上环境配置，别被它吓到，跟上就好。开学先熟悉 Java / Scala 和命令行、尽早搭好 Hadoop / Spark 本地环境，能省掉很多后续麻烦。三个 Project 难度递增，Project 3 占比最大、要留足时间；期末有 hurdle，一定系统复习 MapReduce / Spark 算法设计的手写题型。
...
- **整体难度**：⭐⭐⭐☆☆ — 概念覆盖面广但偏应用、少重理论，对有 INFS1603 基础（先修要求）的商科学生较友好，实践项目是主要投入点。
- **成绩组成**：官方 course outline 页面需登录，2026 具体评估项占比未能核实，以当学期 course outline 为准。
- **建议学习路线**：这门课偏应用、少重理论，对有 INFS1603 底子的商科同学挺友好，选之前把那门课的 SQL 和关系建模弄扎实就行。实践项目围绕真实数据库设计 / 分析场景，重点练需求到 schema 的转化，以及 BI / 数据仓库建模。平时按周完成 lab，别攒到 project 阶段赶工，那样会很累。
...
> 「整体难度 / 建议学习路线」为主观经验参考；UNSW 各 term 的评估占比常有微调，正式选课与评估请以对应 term 的官方 course outline 为准。
